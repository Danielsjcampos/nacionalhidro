...
interface Props {
  ...
  escalas: any[];
  data: string;
  updateVeiculos: (selecionados: any[], escala?: any) => void;
  escala?: any;
}

function statusVeiculos(veiculos: any[], escalas: any[], data: string) {
  return veiculos.map((v: any) => {
    const emUso = (escalas || []).some((e: any) => {
      if (!e.data || !data) return false;
      const mesmaData = e.data.split('T')[0] === data.split('T')[0];
      const temVeiculo = (e.veiculos || e.EscalaVeiculos || []).some(
        (ev: any) => (ev.veiculoId ?? ev.id) === v.id
      );
      return mesmaData && temVeiculo;
    });
...
export default function ModalQuadroVeiculosLogistica({
  modal, handleClose, veiculos, escalas, data, updateVeiculos, escala,
}: Props) {
  ...
  React.useEffect(() => {
    ...
    setLista(statusVeiculos(veiculos, escalas, data).map(v => ({ ...v, _selecionado: false, _manutencao: false })));
    setIniciado(true);
    setBusca('');
  ...
  const filtrados = useMemo(() => {
    return lista.filter(v => {
      const desc = (v.descricao ?? v.modelo ?? '').toLowerCase();
  ...
  const handleConfirm = () => {
    ...
    updateVeiculos(selecionados, escala);
    handleClose();
  };
  ...
  const VeicCard = ({ v }: { v: any }) => (
    ...
    >
      <input type="checkbox" checked={!!v._selecionado} onChange={() => toggleSelect(v.id)}
        className="w-3.5 h-3.5 accent-blue-600 cursor-pointer flex-shrink-0" onClick={e => e.stopPropagation()} />
      <span className={`w-2.5 h-2.5 rounded-full flex-shrink-0 ${STATUS_COLORS[v._status] ?? 'bg-slate-300'}`} />
      <div className="flex-1 min-w-0">
        <p className="text-xs font-bold text-slate-800 truncate">{v.descricao ?? v.modelo} — {v.placa}</p>
        <p className="text-[10px] text-slate-400">{v.tipo ?? ''}</p>
      </div>
      <select
      ...
      <div className="bg-white rounded-2xl w-full max-w-3xl shadow-2xl border border-slate-200 flex flex-col max-h-[90vh]">
        <div className="bg-[#1e3a5f] px-6 py-4 flex items-center justify-between text-white rounded-t-2xl flex-shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {data && <p className="text-[10px] text-blue-300 mt-0.5">{new Date(data).toLocaleDateString('pt-BR')}</p>}
            </div>
          </div>
        ...
        <div className="px-6 py-3 border-b border-slate-200 flex gap-3 items-center flex-shrink-0">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Buscar por descrição ou placa..." value={busca} onChange={e => setBusca(e.target.value)}
              className="w-full pl-9 pr-3 py-2 text-xs border border-slate-200 rounded-lg outline-none focus:border-blue-500" />
          </div>
          <select className="text-xs border border-slate-200 rounded-lg px-3 py-2 outline-none focus:border-blue-500" value={filtroStatus} onChange={e => setFiltroStatus(e.target.value)}>
        ...
        <div className="px-6 py-2 flex gap-4 border-b border-slate-100 flex-shrink-0">
          {Object.entries({ Manutenção: 'bg-red-500', Disponível: 'bg-emerald-500', 'Em uso': 'bg-slate-400' }).map(([label, color]) => (
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <p className="text-[10px] font-black text-emerald-700 uppercase tracking-widest mb-2">Disponíveis ({disponiveis.length})</p>
            ...
            <div className="space-y-2">
              <p className="text-[10px] font-black text-red-700 uppercase tracking-widest mb-2">Em Uso / Manutenção ({indisponiveis.length})</p>
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between rounded-b-2xl flex-shrink-0">
          <span className="text-xs text-slate-500 font-bold">{lista.filter(v => v._selecionado).length} selecionado(s)</span>
          <div className="flex gap-3">
            <button onClick={handleClose} className="px-5 py-2.5 text-xs font-bold text-slate-500 hover:text-slate-700 uppercase tracking-wide transition-colors">Cancelar</button>
            <button onClick={handleConfirm} className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-black uppercase tracking-wide rounded-xl flex items-center gap-2 transition-colors shadow-md">
              <Truck className="w-4 h-4" />Definir Veículos
            </button>
          </div>
        </div>
      </div>
