// ─── Módulo 5 — Logística: Quadro de Funcionários ────────────────────────────
// Wrapper sobre ModalQuadroFuncionariosLogistica com API unificada da spec 5B.
...
// ─── Types ────────────────────────────────────────────────────────────────────
interface Props {
  /** Objeto com cliente, data e lista de funcionários já escalados */
  data: { Cliente?: any; Data?: string; EscalaFuncionarios?: any[] };
  modal: boolean;
  handleClose: () => void;
  funcionarios: any[];
  escalas: any[];
  /** Callback unificado: recebe todos selecionados + data (ou escala) opcional */
  updateFuncionarios: (selecionados: any[], context?: any) => void;
}

// ─── Status logic ─────────────────────────────────────────────────────────────
function calcularStatus(funcionarios: any[], escalas: any[], cliente: any, dataAtual: string) {
  const hoje = new Date();
  ...
  return funcionarios.map((func: any) => {
    const result = {
      ...
      _motivo: '',
      _statusOperacao: 0 as number,
      _selecionado: false,
    };

    // 1. Afastado
    const fimAfastamento = func.FimAfastamento
      ? new Date(func.FimAfastamento)
      : func.fimAfastamento
        ? new Date(func.fimAfastamento)
        : null;

    const motivoAfastamento = func.MotivoAfastamento ?? func.motivoAfastamento;

    if (motivoAfastamento && motivoAfastamento > 0) {
      if (!fimAfastamento || fimAfastamento > hoje) {
        const labels = ['', 'Férias', 'Atestado', 'Pátio', 'Banco de Horas'];
        result._status = 'Afastado';
        result._motivo = labels[motivoAfastamento] ?? 'Afastado';
        result._statusOperacao = motivoAfastamento;
        return result;
      }
    }
    ...
    const escaladoEmOutro = (escalas || []).some((e: any) => {
      ...
      const dataAtualStr = (dataAtual ?? '').split('T')[0];
      if (!dataEscala || !dataAtualStr || dataEscala !== dataAtualStr) return false;
      return (e.funcionarios ?? e.EscalaFuncionarios ?? []).some(
...
export default function ModalFuncionarios({
  data, modal, handleClose, funcionarios, escalas, updateFuncionarios,
}: Props) {
  ...
  React.useEffect(() => {
    ...
    const processados = calcularStatus(funcionarios, escalas, data?.Cliente, dataAtual);
    setLista(processados.map(f => ({ ...f, _selecionado: false })));
    setIniciado(true);
    setBusca('');
    setFiltroStatus('Todos');
  }, [modal, funcionarios, escalas, data]);

  const filtrados = useMemo(() => {
    return lista.filter(f => {
      const nome = (f.nome ?? f.Nome ?? '').toLowerCase();
      const cargo = (f.cargo ?? f.Cargo?.Descricao ?? '').toLowerCase();
  ...
  const disponiveis   = filtrados.filter(f => ['Disponivel', 'Integrado', 'Vencido'].includes(f._status));
  const indisponiveis = filtrados.filter(f => ['Afastado', 'Escalado'].includes(f._status));
  ...
  const handleConfirm = () => {
    ...
    updateFuncionarios(selecionados, data);
    handleClose();
  };

  const clienteNome = data?.Cliente?.RazaoSocial ?? data?.Cliente?.razaoSocial ?? data?.Cliente?.nome ?? '';
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs font-bold text-slate-800 truncate">
          {func.nome ?? func.Nome}
          {func._motivo ? ` (${func._motivo})` : ''}
        </p>
        <p className="text-[10px] text-slate-500 truncate">
          {func.cargo ?? func.Cargo?.Descricao ?? ''}
        </p>
      </div>
      <select
      ...
      <div className="bg-white rounded-2xl w-full max-w-4xl shadow-2xl border border-slate-200 flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between rounded-b-2xl flex-shrink-0">
          ...
          <div className="flex gap-3">
            <button
              onClick={handleClose}
              className="px-5 py-2.5 text-xs font-bold text-slate-500 hover:text-slate-700 uppercase tracking-wide transition-colors"
            >
              Cancelar
            </button>
            <button
