...
import { 
  Plus, Search, Filter, FileText, ChevronRight, Save, X, 
  Trash2, User, Building2, Calendar, Clock, DollarSign, 
  Loader2, Wrench, Settings, Briefcase, Mail, Send, AlertTriangle,
  ClipboardList, Users, History, Check, Truck, MapPin, Gauge
} from 'lucide-react';
...
export default function Logistica() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [escRes, veiRes, cliRes] = await Promise.all([
        axios.get(`http://localhost:3000/logistica/escalas`, { headers: { Authorization: `Bearer ${token}` } }),
  ...
  const handleSave = async () => {
    try {
      ...
      if (selectedItem.id) {
        await axios.patch(`http://localhost:3000/logistica/${endpoint}/${selectedItem.id}`, selectedItem, {
          headers: { Authorization: `Bearer ${token}` }
        });
      } else {
        await axios.post(`http://localhost:3000/logistica/${endpoint}`, selectedItem, {
          headers: { Authorization: `Bearer ${token}` }
        });
      }
  ...
  const handleSendToMaint = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.patch(`http://localhost:3000/logistica/veiculos/${maintModal.veiculo.id}/manutencao`, {
                      ...
                      <tr key={escala.id} className="hover:bg-slate-50 transition-colors group">
                        <td className="px-4 py-3">
                           <div className="flex gap-1">
                              ...
                              <button className="p-2 text-slate-300 hover:text-red-500 hover:bg-white rounded-lg transition-all border border-transparent hover:border-slate-100"><X className="w-3.5 h-3.5" /></button>
                           </div>
                        </td>
                        ...
                        <td className="px-4 py-3">
                           <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest ${
                              escala.status === 'CONCLUIDO' ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-100 text-blue-600'
                           }`}>
                              {escala.status}
                           </span>
                        </td>
                     ...
                     <div key={v.id} className="bg-white border border-slate-200 rounded-3xl p-6 hover:shadow-2xl transition-all group relative overflow-hidden flex flex-col">
                        <div className="absolute top-0 right-0 p-6">
                           <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border shadow-sm ${
                             v.status === 'DISPONIVEL' ? 'bg-emerald-50 text-emerald-600 border-emerald-100 shadow-emerald-500/10' :
                                 ...
                                 <div className="flex-1 h-3 bg-slate-200 rounded-full overflow-hidden">
                                    <div 
                                       className={`h-full rounded-full transition-all duration-1000 ${
                                          Number(v.nivelCombustivel) > 50 ? 'bg-emerald-500' :
                                          Number(v.nivelCombustivel) > 20 ? 'bg-amber-500' : 'bg-red-500'
                                       }`}
                        ...
                        <div className="mt-auto pt-4 border-t border-slate-50 flex gap-2">
                           ...
                           {v.status !== 'MANUTENCAO' && (
                              <button 
                                 ...
                                 className="flex-1 bg-amber-600 hover:bg-amber-700 text-white px-3 py-2.5 rounded-xl text-[10px] font-black uppercase italic tracking-widest transition-all shadow-lg shadow-amber-500/20 flex items-center justify-center gap-2"
                              >
                                 <Wrench className="w-3.5 h-3.5" /> Enviar Oficina
                              </button>
                           )}
                        </div>
                     </div>
                   ...
                   <div className="bg-white p-10 rounded-3xl border border-slate-200 shadow-sm space-y-8 relative overflow-hidden">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                         <div className="space-y-2">
                            ...
                            <input 
                              type="text" 
                              className="w-full bg-slate-50/50 border-2 border-slate-100 rounded-xl px-4 py-3 text-sm font-bold outline-none focus:border-red-500 transition-all"
      ...
      {maintModal.open && (
         <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-50 flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200">
               <div className="bg-amber-600 p-8 flex items-center gap-4 italic">
               ...
               <div className="p-10 space-y-6">
                  <div className="space-y-2">
                     ...
                     <textarea 
                        className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-4 text-sm min-h-[120px] outline-none focus:border-amber-500 font-bold transition-all"
                     ...
                     <div className="grid grid-cols-3 gap-2">
                        {['BAIXA', 'MEDIA', 'ALTA'].map(p => (
                           <button 
                              ...
                              className={`py-2.5 rounded-xl text-[10px] font-black uppercase italic border-2 transition-all ${
                                 maintModal.prioridade === p ? 'bg-amber-600 border-amber-600 text-white shadow-lg' : 'bg-white border-slate-100 text-slate-400 hover:border-amber-200'
                              }`}
                           >
                              {p}
                           </button>
               ...
               <div className="p-6 bg-slate-50 border-t border-slate-200 flex justify-end gap-3">
                  <button onClick={() => setMaintModal({ ...maintModal, open: false })} className="px-6 py-3 text-[10px] font-black uppercase text-slate-400 hover:text-red-500 transition-all italic">Cancelar</button>
                  <button 
                     onClick={handleSendToMaint}
                     className="bg-amber-600 hover:bg-amber-700 text-white px-10 py-3.5 rounded-2xl flex items-center gap-2 transition-all shadow-xl shadow-amber-500/20 text-[10px] font-black uppercase italic tracking-widest"
                  >
