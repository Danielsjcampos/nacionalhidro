...
import { 
  Plus, Loader2, Wrench, Save, 
  AlertTriangle, CheckCircle2, Truck, 
  ArrowRight, ShieldAlert, DollarSign, Calendar,
  Package, X
} from 'lucide-react';
...
export default function Manutencao() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [maintRes, veiRes, prodRes] = await Promise.all([
        api.get('/manutencao'),
  ...
  const handleSave = async () => {
    try {
      const payload = { ...selectedMaint };
      // Incluir peças utilizadas se houver
      if (pecasUtilizadas.length > 0) {
        payload.pecasUtilizadas = pecasUtilizadas.filter((p: any) => p.produtoId && p.quantidade > 0);
      }
      if (selectedMaint.id) {
        await api.patch(`/manutencao/${selectedMaint.id}`, payload);
      } else {
  ...
  // ── Peças helpers ──
  const addPeca = () => setPecasUtilizadas(prev => [...prev, { produtoId: '', quantidade: 1, valorUnitario: 0 }]);
  const removePeca = (idx: number) => setPecasUtilizadas(prev => prev.filter((_, i) => i !== idx));
  const updatePeca = (idx: number, field: string, value: any) => {
    setPecasUtilizadas(prev => {
      const arr = [...prev];
      arr[idx] = { ...arr[idx], [field]: value };
      // Auto-fill valor unitário do catálogo se produto mudou
      if (field === 'produtoId') {
        const prod = produtos.find((p: any) => p.id === value);
        if (prod) arr[idx].valorUnitario = Number(prod.precoCusto) || 0;
      }
      return arr;
    });
  };
  const totalPecas = pecasUtilizadas.reduce((sum, p) => sum + (Number(p.quantidade) || 0) * (Number(p.valorUnitario) || 0), 0);

  const handleLiberar = async (id: string) => {
     try {
        await api.patch(`/manutencao/${id}/liberar`, {});
        fetchData();
     } catch (err) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                 ...
                 <section className="space-y-8 bg-white p-8 rounded-3xl border border-slate-100 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 gap-8">
                       ...
                       <div>
                          <p className="text-[10px] font-black text-amber-600 uppercase tracking-widest italic mb-3 flex items-center gap-2">
                            <Package className="w-3.5 h-3.5" /> Peças do Estoque
                          </p>
                          ...
                          <button type="button" onClick={addPeca} className="text-[10px] font-black text-amber-600 hover:text-amber-700 flex items-center gap-1 mt-1 uppercase tracking-widest">
                            <Plus className="w-3.5 h-3.5" /> Adicionar Peça do Estoque
                          </button>
                          {pecasUtilizadas.length > 0 && (
                            <p className="text-[10px] text-slate-400 mt-2 italic">⚠️ As peças serão descontadas do estoque ao concluir a manutenção.</p>
                          )}
                       </div>
