...
def parse_mysql_insert(sql_line):
    ...
    # We pretend the whole string is inside a giant list: `[ ( ... ), ( ... ) ]`
    # We parse char by char
    i = 0
    while i < len(values_str):
        ...
        if escape:
            ...
            if c == ',' and values_str[i-1] == ')' and values_str[i+1] == '(':
                ...
                current_val = ""
            elif c == '(' and current_val.strip() == '':
                pass # start of row
            elif c == ')' and (i == len(values_str)-1 or values_str[i+1] == ','):
                # end of row
                current_row.append(current_val)
            else:
                current_val += c
...
print("Parsed tables:", {k: len(v) for k, v in tables.items()})
...
for row in tables.get('responsabilidades', []):
    if len(row) > 4:
        ...
        responsabilidades[r_id] = {'desc': desc, 'tipo': tipo, 'importante': importante}

er_to_r = {}
for row in tables.get('equipamento_responsabilidades_responsabilidade_links', []):
    if len(row) >= 3:
        er_to_r[clean(row[1])] = clean(row[2])
...
for eq_id, eq_nome in equipamentos.items():
    ...
    for er_id in eq_to_er.get(eq_id, []):
        ...
        if r_id and r_id in responsabilidades:
            resps.append({
                'descricao': responsabilidades[r_id]['desc'],
                'tipo': responsabilidades[r_id]['tipo'],
                'importante': responsabilidades[r_id]['importante']
            })
